...
| `model` | inherit parent | Default model, or the required first entry of `model_pool` |
| `model_pool` | — | Comma-separated allowed models. The first is the default; a caller may select another pool member, but no outside model |
| `model_diversity_group` | — | In workflows, different agent types in the same group must resolve to different models |
...
Frontmatter is authoritative. If an agent file sets `model`, `thinking`, `max_turns`, `inherit_context`, `run_in_background`, `isolated`, or `isolation`, those values are locked for that agent. `model_pool` is the exception: its first model is the default, and callers can select another member of that pool. `Agent` tool parameters only fill fields the agent config leaves unspecified.
